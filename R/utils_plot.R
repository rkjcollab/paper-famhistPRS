...
# Makes forest plot for linear model/hypothesis 1
make_h1_forest_plot <- function(data, ref, fdr_order) {
  ...
  plot <- ggplot(data_plot, aes(
    ...
    geom_point(
      size = 2.5, shape = 21, colour = "white", stroke = 0.5,
...
# Makes forest plot for survival model/hypothesis 2
make_h2_forest_plot_manuscript <- function(data, ref, term, fdr_order, outcome_order) {
  ...
  plot <- ggplot(
    data_plot,
    aes(
      x = term, y = or, ymin = or_conf_low, ymax = or_conf_high, color = term,
...
# Wraps plot_tt_hr in utils_coxph_tt.R with manuscript-specific styling
